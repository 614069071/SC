...
export default {
  ...
  watch: {
    'data.merchantCode': {
      handler() {
        const { channelNo } = this.reportData;
        ...
        this.getReportData();
        this.getAccountBanlList(channelNo);
    ...
    'performInfoData.merchantProvince'(nv, ov) {
      ...
      const data = { parentAreaId: nv, channelCode: this.reportData.channelNo };
    ...
    'performInfoData.suiXinBank'(nv, ov) {
      ...
      const data = {
        channelCode: this.reportData.channelNo,
        queryBean: {
          bankCode: nv,
  ...
  methods: {
    // 获取详细信息
    getReportData() {
      const { channelNo, merchantCode } = this.reportData;
      const data = { merchantCode };

      this.getComChannelBankArea(channelNo);

      // 获取对应渠道地址
      fetch
        .comChannelMerchantAarea({ channelCode: channelNo })
        .then(({ data = [] }) => {
    ...
    // 银行开户省份
    getComChannelBankArea(channelCode) {
      fetch.comChannelBankArea({ channelCode }).then(res => {
        this.bankProvinces = res.data || [];
      });
    },
    // 商户行业信息
    getComChannelIndustryCategory() {
      fetch.comChannelIndustry().then(res => {
      ...
      this.checkFormSubmit(form, () => {
        this.uploadPerformImage(taskCode => {
          ...
          const mapData = {
            ...
            tdMerchantReport: {
              channelNo: this.reportData.channelNo,
    ...
    uploadPerformImage(callback) {
      ...
      fd.append('merchantCode', this.merchantCode);
    ...
    // 获取开户子银行
    getAccountBanlList(code) {
      fetch.comBankList({ channelCode: code }).then(({ data = [] }) => {
